...
class HelperFunctions:
    ...
    def load_interface_data(key: str, filename: str) -> Any:
        ...
        try:
            with file_path.open("r", encoding="utf-8") as file:
                data = json.load(file)
                # logger.info(f"Загружены данные из файла {file_path}")
                return data.get(key)
    ...
    def load_oids(file_path: str) -> dict:
        ...
        try:
            with file_path.open("r", encoding="utf-8") as file:
                # logger.info(f"Загружены данные устройства из файла {file_path}")
                return json.load(file)
        except FileNotFoundError:
            logger.error(LogMessages.FILE_NOT_FOUND.value.format(file_path=file_path))
            return {}
        except json.JSONDecodeError:
            logger.error(LogMessages.JSON_DECODE_ERROR.value.format(file_path=file_path))
            return {}
