...
async def generate_pagination_keyboard(page: int, total_pages: int) -> InlineKeyboardMarkup:
    ...
    start_page, end_page = max(1, page - 2), min(total_pages, page + 2)
    ...
    navigation_buttons = [
        ...
        *[
            ...
            if i != page else InlineKeyboardButton(text=f"⟨ {i} ⟩", callback_data="current_page")
            for i in range(start_page, end_page + 1)
        ],
        InlineKeyboardButton(text="Вперёд ➡️", callback_data=f"page_{page + 1}") if page < total_pages else None
    ]
    
    # Удаляем None и добавляем кнопку назад
...
async def view_users(message: Message, state: FSMContext):
    ...
    try:
        ...
        # Устанавливаем текущую страницу и сохраняем данные
        page = 1
