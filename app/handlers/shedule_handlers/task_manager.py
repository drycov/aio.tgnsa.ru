...
from app.keyboards import priority_kb, in_back_keyboard
from app.models import Task, PriorityLevel, User
from app.utils import CalendarMarkup, HelperFunctions
...
async def view_tasks(message: types.Message, created_by: Optional[int] = None, assigned_to: Optional[int] = None,
    ...
    if task_id:
        ...
        tasks = all_tasks if isinstance(all_tasks, dict) else {}
    ...
    tasks_info = "\n\n".join([
        f"Задача {idx + 1 + start_idx}:\nДата начала: {task.get('date', 'не задано')}\n"
        ...
        f"Исполнитель: {task.get('assigned_to', 'не назначен')}\n"
        f"Автор: {task.get('created_by', 'не указан')}\n"
        f"Для редактирования и просмотра: /vt_{key}\n"
        for idx, (key, task) in enumerate(tasks_to_display) if isinstance(task, dict)
    ])
...
# Обработчики команд
@router.message(F.text.startswith("/vt_"))
